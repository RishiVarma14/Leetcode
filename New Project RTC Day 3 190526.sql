...
#Use Case: Find drivers from the SAME city with SAME experience years

SELECT 
    A.driver_name   AS Driver_1,
    B.driver_name   AS Driver_2,
    A.city          AS City,
    A.experience_years AS Experience
FROM drivers A
JOIN drivers B
    ON A.city = B.city
...
# Use Case: Find drivers earning MORE than another driver in same city
SELECT 	
    A.driver_name  AS Higher_Paid_Driver,
    A.salary       AS Higher_Salary,
    B.driver_name  AS Lower_Paid_Driver,
    B.salary       AS Lower_Salary,
    A.city
FROM drivers A
JOIN drivers B
    ON A.city = B.city
